...
describe('GET /user/me', () => {
    ...
    it('should return a 401 if not authenticated', (done) => {
        request(app)
        .get('/users/me')
        .set('x-auth', 'sdfsdfsgsfgfsgfsgsdfsdf')
        .expect(401)
        .expect((res) => {
...
describe('POST /users', () => {
    ...
    it('should not create user if email already in use', (done) => {
        var seedEmail = users[0].email;
        var seedPassword = 'abc123!@#';

        request(app)
        .post('/users')
        .send({seedEmail, seedPassword})
        .expect(400)
        .end(done);
    });
});
